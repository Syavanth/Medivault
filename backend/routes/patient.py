...
def create_appointment():
    try:
        ...
        # Optional: check availability_slots (if populated)
        if doctor.availability_slots:
            try:
                # assume stored as JSON list or comma-separated values
                import json
                slots = None
                try:
                    slots = json.loads(doctor.availability_slots)
                except Exception:
                    slots = [s.strip() for s in doctor.availability_slots.split(',') if s.strip()]

                # accept match by isoformat or date + time substring
                slot_matches = any(str(appt_dt) in str(s) or appt_dt.isoformat() == str(s) or appt_dt.strftime('%Y-%m-%dT%H:%M') == str(s) for s in slots)
                if not slot_matches:
                    # not matching a known slot; still allow but warn
                    logger.info(f"Requested slot {appt_dt.isoformat()} not in doctor's availability")
            except Exception:
                logger.info('Could not parse doctor availability slots')
        ...
        appointment_data = {
            ...
            'purpose': new_appt.purpose if hasattr(new_appt, 'purpose') else purpose,
